...
class Repository extends Singleton
{
     ...
     * @return Builder
     */
    public function builder(bool $fresh = false): Builder
    {
        if (!isset($this->builder) || $fresh) $this->createBuilder();
    ...
    public function createBuilder(?string $table_name = null, ?string $connection_name = null): self
    {
        ...
        if (!isset($this->table_name)) $this->table($this->defaultTableName());


        if (!is_null($connection_name)) $this->connection($connection_name);
        if (!isset($this->connection_name)) $this->connection($this->defaultConnectionName());
    ...
    {
        return camel_to_snake(get_class_name(get_called_class(), 'Repository'));
    }
    ...
    /**
     * Define default table name for this repository
     *
     * @return string
     */
    protected function defaultConnectionName(): string
    {
        return camel_to_snake(get_class_current_directory(get_called_class()));
    }
     ...
     * @return $this
     */
    protected function resolveSpecialParameters(array $special_parameters = []): self
    {
        return $this;
    }
    ...
    protected function buildWhereClause(string $column, string $parameter): void
    {
        ...
        if ($operator === 'null') {
            $value == 'true' ? $this->builder()->whereNull($column) : $this->builder()->WhereNotNull($column);
        } else if (strtoupper($operator) === 'in') {
            if (in_array('null', $value)) {
                unset($value[array_search('null', $value)]);
                $this->builder()->where(function ($q) use ($column, $value) {
    ...
    {
        return $this->wrapIntoTransaction(function () use ($data, $return_data) {
    ...
    {
        return $this->wrapIntoTransaction(function () use ($parameters, $return_data, $force_empty_where) {
    ...
    {
        return $this->wrapIntoTransaction(function () use ($where, $updated_data, $return_data, $force_empty_where) {
    ...
    {
        $this->wrapIntoTransaction(function () {
